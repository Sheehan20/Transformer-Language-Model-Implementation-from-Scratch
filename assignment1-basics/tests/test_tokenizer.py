...
def safe_token_to_bytes(token, gpt2_byte_decoder):
    """安全地将token转换为bytes，跳过有问题的字符"""
    try:
        return gpt2_byte_decoder[token]
    except KeyError:
        # 对于不在映射表中的字符，使用UTF-8编码的第一个字节
        return ord(token.encode('utf-8')[0])
...
def get_tokenizer_from_vocab_merges_path(
    ...
    for gpt2_vocab_item, gpt2_vocab_index in gpt2_vocab.items():
        try:
            vocab[gpt2_vocab_index] = bytes([gpt2_byte_decoder[token] for token in gpt2_vocab_item])
        except KeyError as e:
            print(f"Skipping problematic vocab item: {gpt2_vocab_item}, Error: {e}")
            continue
    ...
    for merge_token_1, merge_token_2 in gpt2_bpe_merges:
        try:
            ...
            print(f"Skipping problematic merge: {merge_token_1} + {merge_token_2}, Error: {e}")
            continue
